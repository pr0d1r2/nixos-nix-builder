...
| B18 | 2026-07-14 | `lefthook.yml` added `markdownlint-agentic` command but `flake.nix` never declared `nix-lefthook-markdownlint-agentic` input — CI failed with `lefthook-markdownlint-agentic: No such file or directory` (exit 127) | fixed: declare input + add package to dev shell, relock |
| B19 | 2026-07-14 | `lefthook.yml` `exclude` used a regex string, which lefthook ignores under `--all-files` — `markdownlint-agentic` linted the exempt `SPEC.md` and MD013 failed on its >500-char constraint lines | fixed: express `exclude` as a glob list (per lefthook/glob skill) |
| B20 | 2026-07-22 | migrated `flake.nix` declared only `base` and `nix` lefthook fragments, so fidelity rejected the detected shell, ASCII, Markdown, and YAML fragments; the standalone confirm app omitted their wrappers, and the newly activated shell check exposed stale case indentation | fixed: declare the complete detected fragment set, add the materialization packages to confirm's runtime, align EditorConfig with the pinned formatter, and reformat shell scripts |
| B21 | 2026-08-03 | `flake.nix` constructed outputs inline, violating the strict thin-manifest guardrail | fixed: delegate output construction to `nix/outputs.nix` |
| B22 | 2026-08-18 | `flake.nix` custom `checksFor` and `actionlint` expressions were not formatted with the pinned nixfmt version, causing guardrails `nixfmt-check` to fail | fixed: apply pinned nixfmt formatting |
...
| B25 | 2026-09-24 | guardrails workflow invokes `lefthook-tdd-order-bats` for tracked Bats tests, but the consumer fragment list omitted `bats`, so the wrapper was absent and CI exited 127 | fixed: include the `bats` fragment in the flake consumer manifest |
| B26 | 2026-09-24 | `nix/outputs.nix` kept a separate fragment manifest without `bats`, so generated dev-shell/check tooling still omitted `lefthook-tdd-order-bats` after the top-level manifest was fixed | fixed: include `bats` in the output materialization manifest |
