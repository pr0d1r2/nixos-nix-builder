...
| B20 | 2026-09-23 | Guardrails Bats suite passed, then CI exited 127 because `lefthook-tdd-order-bats` was not materialized; the pinned `set-and-setting` revision lacked the newer Bats integration | update `set-and-setting` and its wrapper inputs, and declare the `bats` fragment |
...
| B23 | 2026-09-22 | materialized `actionlint` lefthook wrapper was enabled, but `actionlint` was absent from the dev shell runtime packages, causing CI/pre-commit to exit 127 | fixed: add `pkgs.actionlint` to the dev shell packages |
